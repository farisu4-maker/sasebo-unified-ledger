...
export const PaymentForm: React.FC<PaymentFormProps> = ({ member, allMembers, feeItems, transactions, onClose, onSubmit }) => {
  ...
  // 「登録する」を押した時点ではまだ確定させず、内容を要約したモーダルを
  // 一度挟んでから、明示的な「この内容で登録する」でようやくonSubmitを呼ぶ。
  const [showConfirm, setShowConfirm] = useState(false);
  const [confirmOrg, setConfirmOrg] = useState<string>('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // ここではまだ登録せず、確認モーダルを表示するだけ
    setConfirmOrg(targetOrg || '');
    setShowConfirm(true);
  };
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-6">
            ...
            <div className="space-y-2 text-sm bg-gray-50 rounded-lg p-4 border border-gray-200">
              <div className="flex justify-between">
                <span className="text-gray-500">拳士</span>
                <span className="font-bold text-gray-800">{member.name}</span>
              </div>
              {member.organization === '両方' && (
                <div className="flex justify-between">
                  <span className="text-gray-500">登録先</span>
                  <span className="font-bold text-gray-800">{confirmOrg}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-gray-500">費目</span>
                <span className="font-bold text-gray-800">{selectedItem}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">入金処理日</span>
                <span className="font-bold text-gray-800">{paymentDate}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">決済手段</span>
                <span className="font-bold text-gray-800">{paymentMethod}</span>
              </div>
              <div>
                <span className="text-gray-500 block mb-1">対象月（{targetMonths.length}ヶ月）</span>
                <div className="flex flex-wrap gap-1">
                  {targetMonths.map(m => {
                    const [y, mo] = m.split('-');
                    return (
                      <span key={m} className="bg-indigo-100 text-indigo-700 text-xs font-medium px-2 py-0.5 rounded-full">
              ...
              <div className="border-t border-gray-200 pt-2 mt-2 flex justify-between items-center">
                ...
                <span className="text-xl font-bold text-indigo-700">¥{(amount * targetMonths.length).toLocaleString()}</span>
              </div>
              {targetMonths.length > 1 && (
                <p className="text-[11px] text-gray-400">
                  （{amount.toLocaleString()}円 × {targetMonths.length}ヶ月分、独立した{targetMonths.length}件のデータとして登録されます）
                </p>
              )}
            </div>
            ...
            <div className="mt-5 flex justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 rounded-md shadow-sm text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 transition-colors"
              >
                この内容で登録する
              </button>
            </div>
